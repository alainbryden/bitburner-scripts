...
async function tryGet4SApi(ns, playerStats, bitnodeMults, budget, allStockSymbols) {
    ...
    if (!playerStats.has4SData) {
        if (await getNsDataThroughFile(ns, 'ns.stock.purchase4SMarketData()', '/Temp/purchase-4s.txt'))
            log(ns, `SUCCESS: Purchased 4SMarketData for ${formatMoney(cost4sData)} (At ${formatDuration(player.playtimeSinceLastBitnode)} into BitNode)`, true, 'success');
        else
    ...
    if (await getNsDataThroughFile(ns, 'ns.stock.purchase4SMarketDataTixApi()', '/Temp/purchase-4s-api.txt')) {
        log(ns, `SUCCESS: Purchased 4SMarketDataTixApi for ${formatMoney(cost4sApi)} (At ${formatDuration(player.playtimeSinceLastBitnode)} into BitNode)`, true, 'success');
        return true;
    } else {
