...
const argsSchema = [
    ...
    ['click-sleep-time', 1], // Time to sleep in milliseconds before and after clicking any button (or setting text). Increase if clicks don't appear to be "taking effect".
    ['find-sleep-time', 5], // Time to sleep in milliseconds before trying to find any element on screen. Increase if you are frequently getting errors detecting elements that should be on screen.
...
export async function main(ns) {
     ...
     * Checks explicitly if focus was stolen by (e.g.) faction|company work|studying|training and send that work to the background
     * @param {boolean} throwErrorIfNot (default true) If true, and we are no longer on the casino page, throws an exception.
     *                  If false, it will log a warning, try to stop any focus work work (up to `retries` times), then return true.
    ...
    async function checkStillAtCasino(throwErrorIfNot = true, retries = 0, silent = false) {
        // Check whether we're still on the casino page
        let stillAtCasino = await tryfindElement(ns, "//h4[text()='Iker Molina Casino']");
        ...
        let baseMessage = "It looks like " + (btnUnfocus ? "something stole focus" :
            "the user or another script navigated away from the casino page") +
            " while casino.js was trying to automate the casino.";
        if (throwErrorIfNot) // If we weren't instructed to stop whatever took focus, raise an error
        ...
        log(ns, `WARNING: ${baseMessage}` + (btnUnfocus ? `\nTrying to un-focus it so we can keep going...` : ''), false, 'warning');
        if (!btnUnfocus) return false; // If we have no unfocus button to click, we can return now           
        ...
        return await checkStillAtCasino(retries <= 0 /* if out of retries, throw error on next failure */, retries);
    }
    ...
    async function checkForKickedOut() {
        let closeModal;
        do {
            const kickedOut = await tryfindElement(ns, "//span[contains(text(), 'Alright cheater get out of here')]");
            ...
            let closeModal = await tryfindElement(ns, "//button[contains(@class,'closeButton')]");
    ...
    while (true) {
        ...
        try {
            ...
            await checkStillAtCasino(false /** Don't throw an error, we can tolerate focus stealing (for now) */, 3, true);
            ...
            if (ns.getPlayer().city != "Aevum") {
                ...
                if (!travelled) {
                    await click(await findRequiredElement(ns, "//div[@role='button' and ./div/p/text()='Travel']"));
                    await click(await findRequiredElement(ns, "//span[contains(@class,'travel') and ./text()='A']"));
                }
                if (ns.getPlayer().city == "Aevum")
            ...
            try { // Try to do this without SF4, because it's faster and doesn't require a temp script to be cleaned up below
                ...
                await click(await findRequiredElement(ns, "//div[(@role = 'button') and (contains(., 'City'))]"));
                await click(await findRequiredElement(ns, "//span[@aria-label = 'Iker Molina Casino']"));
                ...
                catch { throw new Error("Failed to travel to the casino both using UI navigation and using SF4 as a fall-back."); }
            }
            // Step 2.4: Try to start the blackjack game
            const blackjack = await findRequiredElement(ns, "//button[contains(text(), 'blackjack')]");
            ...
            if (ns.ls("home", "Temp/").length > 0) { // Do a little clean-up to speed up save/load.
                ...
                if (await tryfindElement(ns, "//p[contains(text(), 'Count:')]", 10)) { // If this works, we're still allowed in
                    ...
                    if (await checkForKickedOut()) return onCompletion(ns, false); // We appear to have previously been kicked out
        ...
        while (true) {
            ...
            if (!btnHit && !btnStay) {
                ...
                if (await checkForKickedOut()) // Were we kicked out of the casino?
                    return onCompletion(ns); // This is a good thing!
            ...
            while (winLoseTie == null) {
                ...
                try {
                    ...
                    // We can't get kicked out mid-game, but we should check if we lost focus (hence the error)
                    await checkStillAtCasino();
...
function onCompletion(ns, kickedOutAfterPlaying = true) {
...
async function click(elem) {
    if (elem === null || elem === undefined)
        throw new Error("click was called on a null element. This means the prior element detection failed, but was assumed to have succeeded.");
    let sleepDelay = options['click-sleep-time'];
    if (sleepDelay > 0) await _ns.sleep(sleepDelay);
    await elem[Object.keys(elem)[1]].onClick({ isTrusted: true });
    if (sleepDelay > 0) await _ns.sleep(sleepDelay);
}
async function setText(input, text) {
    if (input === null || input === undefined)
        throw new Error("setText was called on a null element. This means the prior element detection failed, but was assumed to have succeeded.");
    let sleepDelay = options['click-sleep-time'];
    if (sleepDelay > 0) await _ns.sleep(sleepDelay);
 ...
 * @param {null|number} retries (default null) The number of times to retry. By default,
 *                              if expectFailure = true, defaults to 10, otherwise
 *                              if expectFailure = false, retry up to 4 times to be sure the element isn't present.
 * @param {string?} customErrorMessage (optional) A custom error message to replace the default on failure. */
async function internalfindWithRetry(ns, xpath, expectFailure, retries = null, customErrorMessage = null) {
    try {
        if (options['enable-logging'])
        ...
        let attempts = 0, retryDelayMs = 5; // starting retry delay (ms), will be increased with each attempt
        // Always give the game some time to render an item before we try to find it on screen
        ...
        while (attempts++ <= maxRetries) {
            ...
            if (attempts > 1) {
                if (expectFailure) {
                    if (options['enable-logging'])
                        log(ns, `Warning: Element doesn't appear to be present, moving on...`, false);
                }
                await ns.sleep(retryDelayMs);
                retryDelayMs *= 2; // back-off rate (increases next sleep time before retrying)
                retryDelayMs = Math.min(retryDelayMs, 500); // No use in letting this exceed 500 ms
            }
...
async function getDealerCount(ns) {
    const text = await findRequiredElement(ns, "//p[contains(text(), 'Dealer')]/..").innerText.substring(8, 9);
