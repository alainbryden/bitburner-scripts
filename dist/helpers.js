...
function parseShortNumber(text = "0") {
...
function formatDateTime(datetime) { return datetime.toISOString(); }
...
/** @param {NS} ns **/
function disableLogs(ns, listOfLogs) { ['disableLog'].concat(...listOfLogs).forEach(log => checkNsInstance(ns, '"disableLogs"').disableLog(log)); }
...
function getFnRunViaNsRun(ns) { return checkNsInstance(ns, '"getFnRunViaNsRun"').run; }

/** @param {NS} ns
 *  Use where a function is required to run a script and you have already referenced ns.exec in your script **/
function getFnRunViaNsExec(ns, host = "home") {
...
/** @param {NS} ns
 *  Use where a function is required to run a script and you have already referenced ns.run in your script  */
function getFnIsAliveViaNsIsRunning(ns) { return checkNsInstance(ns, '"getFnIsAliveViaNsIsRunning"').isRunning; }

/** @param {NS} ns
 *  Use where a function is required to run a script and you have already referenced ns.ps in your script  */
function getFnIsAliveViaNsPs(ns) {
 ...
 **/
async function getNsDataThroughFile(ns, command, fName, args = [], verbose = false, maxRetries = 5, retryDelayMs = 50) {
 ...
 * @param {args=} args - args to be passed in as arguments to command being run as a new script.
 **/
async function getNsDataThroughFile_Custom(ns, fnRun, fnIsAlive, command, fName, args = [], verbose = false, maxRetries = 5, retryDelayMs = 50) {
 ...
 */
async function runCommand(ns, command, fileName, args = [], verbose = false, maxRetries = 5, retryDelayMs = 50) {
 ...
 * @param {args=} args - args to be passed in as arguments to command being run as a new script.
 **/
async function runCommand_Custom(ns, fnRun, command, fileName, args = [], verbose = false, maxRetries = 5, retryDelayMs = 50) {
 ...
 * @param {bool=} verbose - (default false) If set to true, pid and result of command are logged.
 **/
async function waitForProcessToComplete(ns, pid, verbose) {
...
async function getActiveSourceFiles(ns, includeLevelsFromCurrentBitnode = true) {
...
async function getActiveSourceFiles_Custom(ns, fnGetNsDataThroughFile, includeLevelsFromCurrentBitnode = true) {
...
function checkNsInstance(ns, fnName = "this function") { if (!ns.print) throw `The first argument to ${fnName} should be a 'ns' instance.`; return ns; }

export { autoRetry, checkNsInstance, disableLogs, formatDateTime, formatDuration, formatMoney, formatNumber, formatNumberShort, formatRam, getActiveSourceFiles, getActiveSourceFiles_Custom, getFilePath, getFnIsAliveViaNsIsRunning, getFnIsAliveViaNsPs, getFnRunViaNsExec, getFnRunViaNsRun, getNsDataThroughFile, getNsDataThroughFile_Custom, hashCode, log, parseShortNumber, pathJoin, runCommand, runCommand_Custom, scanAllServers, tryGetBitNodeMultipliers, tryGetBitNodeMultipliers_Custom, waitForProcessToComplete, waitForProcessToComplete_Custom };
